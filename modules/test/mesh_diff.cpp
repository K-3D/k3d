	...
	bool get_equal()
	{
		...
		for(k3d::iproperty_collection::properties_t::const_iterator prop = properties.begin(); prop != properties.end(); ++prop)
		{
			...
			if(property.property_type() == typeid(k3d::mesh*))
			{
				if(first_mesh)
				{
					if(const k3d::mesh* const mesh = boost::any_cast<k3d::mesh*>(k3d::property::pipeline_value(document().pipeline(), property)))
					{
				...
				{
					first_mesh = boost::any_cast<k3d::mesh*>(k3d::property::pipeline_value(document().pipeline(), property));
				}
			}
		}
