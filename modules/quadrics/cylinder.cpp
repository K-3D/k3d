...
public:
	cylinder(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_transformation(init_owner(*this) + init_name("transformation") + init_label(_("Transformation")) + init_description(_("Transformation matrix used to position / orient / scale the output cylinder.")) + init_value(k3d::identity3D())),
		m_radius(init_owner(*this) + init_name("radius") + init_label(_("Radius")) + init_description(_("Controls the radius of the output cylinder.")) + init_value(1.0) + init_step_increment(0.1) + init_units(typeid(k3d::measurement::distance))),
		m_z_min(init_owner(*this) + init_name("z_min") + init_label(_("Z Min")) + init_description(_("Defines the cylinder length along the -Z axis.")) + init_value(-1.0) + init_step_increment(0.1) + init_units(typeid(k3d::measurement::distance))),
		m_z_max(init_owner(*this) + init_name("z_max") + init_label(_("Z Max")) + init_description(_("Defines the cylinder length along the +Z axis.")) + init_value(1.0) + init_step_increment(0.1) + init_units(typeid(k3d::measurement::distance))),
		...
		m_color(init_owner(*this) + init_name("color") + init_label(_("Color")) + init_description(_("Controls the color of the output cylinder.")) + init_value(k3d::color(1, 1, 1)))
	{
	...
	void on_update_mesh_topology(k3d::mesh& Output)
	{
		...
		boost::scoped_ptr<k3d::cylinder::writable_primitive> primitive(k3d::cylinder::create(Output));
