...
public:
	paraboloid(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_transformation(init_owner(*this) + init_name("transformation") + init_label(_("Transformation")) + init_description(_("Transformation matrix used to position / orient / scale the output paraboloid.")) + init_value(k3d::identity3D())),
