...
public:
	disk(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_transformation(init_owner(*this) + init_name("transformation") + init_label(_("Transformation")) + init_description(_("Transformation matrix used to position / orient / scale the output disk.")) + init_value(k3d::identity3D())),
		...
		m_color(init_owner(*this) + init_name("color") + init_label(_("Color")) + init_description(_("Controls the color of the output disk.")) + init_value(k3d::color(1, 1, 1)))
	{
