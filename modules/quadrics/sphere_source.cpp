...
public:
	sphere_source(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_radius(init_owner(*this) + init_name("radius") + init_label(_("Radius")) + init_description(_("Controls the radius the output sphere.")) + init_value(1.0)),
		m_z_min(init_owner(*this) + init_name("z_min") + init_label(_("Z Min")) + init_description(_("")) + init_value(-1.0)),
		m_z_max(init_owner(*this) + init_name("z_max") + init_label(_("Z Max")) + init_description(_("")) + init_value(1.0)),
		m_sweep_angle(init_owner(*this) + init_name("sweep_angle") + init_label(_("Sweep Angle")) + init_description(_("")) + init_value(k3d::pi_times_2())),
	...
	{
		static k3d::document_plugin_factory<sphere_source > factory(
...
private:
	...
	k3d_data(k3d::double_t, immutable_name, change_signal, with_undo, local_storage, no_constraint, writable_property, with_serialization) m_radius;
	k3d_data(k3d::double_t, immutable_name, change_signal, with_undo, local_storage, no_constraint, writable_property, with_serialization) m_z_min;
	k3d_data(k3d::double_t, immutable_name, change_signal, with_undo, local_storage, no_constraint, writable_property, with_serialization) m_z_max;
	k3d_data(k3d::double_t, immutable_name, change_signal, with_undo, local_storage, no_constraint, writable_property, with_serialization) m_sweep_angle;
	k3d_data(k3d::color, immutable_name, change_signal, with_undo, local_storage, no_constraint, writable_property, with_serialization) m_color;
};

k3d::iplugin_factory& sphere_source_factory()
{
	return sphere_source::get_factory();
}
