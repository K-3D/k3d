...
public:
	position(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_output(init_owner(*this) + init_name("output_matrix") + init_label(_("Output matrix")) + init_description(_("Read only")) + init_slot(sigc::mem_fun(*this, &position::output_value)))
	{
	...
	{
		return m_input.value() * m_space.value() * translation3D(k3d::vector3(m_x.value(), m_y.value(), m_z.value())) * k3d::inverse(m_space.value());
	}
