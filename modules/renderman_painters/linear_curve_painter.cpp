...
public:
	...
	{
		if(!k3d::dev::validate_linear_curve_groups(Mesh))
			return;

		const k3d::dev::mesh::indices_t& first_curves = *Mesh.linear_curve_groups->first_curves;
		const k3d::dev::mesh::counts_t& curve_counts = *Mesh.linear_curve_groups->curve_counts;
		const k3d::dev::mesh::bools_t& periodic_curves = *Mesh.linear_curve_groups->periodic_curves;
		const k3d::dev::mesh::materials_t& materials = *Mesh.linear_curve_groups->materials;
		const k3d::dev::mesh::named_arrays& constant_data = Mesh.linear_curve_groups->constant_data;
		...
		const k3d::dev::mesh::named_arrays& uniform_data = Mesh.linear_curve_groups->uniform_data;
		...
		for(size_t group = group_begin; group != group_end; ++group)
		{
			...
			RenderState.engine.RiCurvesV("linear", ri_point_counts, periodic_curves[group] ? "periodic" : "nonperiodic", ri_parameters);
		}
	}
