...
public:
	...
	void on_update_bitmap(const k3d::bitmap& Input, k3d::bitmap& Output)
	{
		...
		document().pipeline_profiler().start_execution(*this, "Copy to Device");
		timer.restart();
		bitmap_copy_data_from_host_to_device(inputPixels, Input.width(), Input.height());
		m_host_to_device_time.set_value(timer.elapsed());
		document().pipeline_profiler().finish_execution(*this, "Copy to Device");
		
		// perform the calculation
		document().pipeline_profiler().start_execution(*this, "Execute Kernel");
		timer.restart();
		...
		document().pipeline_profiler().finish_execution(*this, "Execute Kernel");
