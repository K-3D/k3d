...
void sds_cache::on_execute(const k3d::mesh& Mesh, k3d::inode* Painter)
{
	...
	for(k3d::mesh::primitives_t::const_iterator primitive = input_with_normals.primitives.begin(); primitive != input_with_normals.primitives.end(); ++primitive)
	{
		...
		k3d::mesh::normals_t& vertex_normals = input_with_normals.point_attributes.create("sds_normals", new k3d::mesh::normals_t(points.size()));
		for(k3d::uint_t face = face_begin; face != face_end; ++face)
		{
		...
		m_caches[primitive->get()].update_mesh(points, *polyhedron, input_with_normals.point_attributes, face_selections, Painter);
	}
}
