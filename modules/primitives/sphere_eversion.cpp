...
public:
	sphere_eversion_implementation(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_latitudinal_patches(init_owner(*this) + init_name("latitudinal_patches") + init_label(_("Latitudinal patches")) + init_description(_("Latitudinal Patches Per Hemisphere")) + init_value(6) + init_constraint(constraint::minimum(2)) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar))),
		m_longitudinal_patches(init_owner(*this) + init_name("longitudinal_patches") + init_label(_("Longitudinal patches")) + init_description(_("Longitudinal Patches Per Strip")) + init_value(4) + init_constraint(constraint::minimum(2)) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar))),
		m_strips(init_owner(*this) + init_name("strips") + init_label(_("Strips")) + init_description(_("Strip Number")) + init_value(8) + init_constraint(constraint::minimum(1)) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar))),
		m_display_strips(init_owner(*this) + init_name("display_strips") + init_label(_("Display strips")) + init_description(_("Display strip Number")) + init_value(8) + init_constraint(constraint::minimum(1)) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar))),
