...
public:
	lissajous_curve_implementation(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_mamp(init_owner(*this) + init_name("mamp") + init_label(_("Modulation Amplitude")) + init_description(_("Modulation amplitude")) + init_value(0.0) + init_step_increment(0.01) + init_units(typeid(k3d::measurement::distance))),
		m_mfreq(init_owner(*this) + init_name("mfreq") + init_label(_("Modulation Frequency")) + init_description(_("Modulation frequency")) + init_value(20.0) + init_step_increment(0.1) + init_units(typeid(k3d::measurement::scalar))),
