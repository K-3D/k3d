...
void gl_draw_2d_widgets(k3d::icamera& Camera, const k3d::rectangle& WindowRect, const k3d::rectangle& CameraRect, const bool DrawFrustum, const bool DrawCropWindow, const bool DrawSafeZone, const bool DrawAimPoint, k3d::idocument& Document)
{
	...
	if(DrawCropWindow)
	{
		const double left = boost::any_cast<double>(k3d::property::pipeline_value(Document.dag(), Camera.crop_window().crop_left()));
		const double right = boost::any_cast<double>(k3d::property::pipeline_value(Document.dag(), Camera.crop_window().crop_right()));
		const double bottom = boost::any_cast<double>(k3d::property::pipeline_value(Document.dag(), Camera.crop_window().crop_bottom()));
		const double top = boost::any_cast<double>(k3d::property::pipeline_value(Document.dag(), Camera.crop_window().crop_top()));
...
public:
	...
	void calculate_projection(k3d::icamera& Camera, const unsigned long PixelWidth, const unsigned long PixelHeight, k3d::rectangle& WindowRect, k3d::rectangle& CameraRect, double& Near, double& Far, bool& Orthographic)
	{
		...
		if(k3d::iperspective* const perspective = dynamic_cast<k3d::iperspective*>(&Camera.projection()))
		{
			...
			CameraRect.left = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), perspective->left()));
			CameraRect.right = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), perspective->right()));
			if(CameraRect.right < CameraRect.left)
				std::swap(CameraRect.left, CameraRect.right);

			CameraRect.top = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), perspective->top()));
			CameraRect.bottom = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), perspective->bottom()));
			if(CameraRect.top < CameraRect.bottom)
				std::swap(CameraRect.top, CameraRect.bottom);

			Near = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), perspective->near()));
			Far = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), perspective->far()));
		...
		else if(k3d::iorthographic* const orthographic = dynamic_cast<k3d::iorthographic*>(&Camera.projection()))
		{
			...
			CameraRect.left = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), orthographic->left()));
			CameraRect.right = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), orthographic->right()));
			if(CameraRect.right < CameraRect.left)
				std::swap(CameraRect.left, CameraRect.right);

			CameraRect.top = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), orthographic->top()));
			CameraRect.bottom = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), orthographic->bottom()));
			if(CameraRect.top < CameraRect.bottom)
				std::swap(CameraRect.top, CameraRect.bottom);

			Near = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), orthographic->near()));
			Far = boost::any_cast<double>(k3d::property::pipeline_value(document().dag(), orthographic->far()));
