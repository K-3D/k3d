...
public:
	bitmap_checker(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_check_width(init_owner(*this) + init_name("check_width") + init_label(_("Check width")) + init_description(_("Check (rectangle) width")) + init_value(8L) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar)) + init_constraint(constraint::minimum(1L))),
		m_check_height(init_owner(*this) + init_name("check_height") + init_label(_("Check height")) + init_description(_("Check (rectangle) height")) + init_value(8L) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar)) + init_constraint(constraint::minimum(1L))),
	...
	void on_create_bitmap(k3d::bitmap& Bitmap)
	{
		...
		const k3d::pixel_size_t check_width = m_check_width.value();
		const k3d::pixel_size_t check_height = m_check_height.value();
		const k3d::color color1 = m_color1.value();
		const k3d::color color2 = m_color2.value();
...
private:
	...
	k3d_data(long, immutable_name, change_signal, with_undo, local_storage, with_constraint, measurement_property, with_serialization) m_check_width;
	k3d_data(long, immutable_name, change_signal, with_undo, local_storage, with_constraint, measurement_property, with_serialization) m_check_height;
