...
private:
	bool render(k3d::icamera& Camera, k3d::irender_frame& Frame, const k3d::filesystem::path& OutputImagePath, const bool VisibleRender, k3d::ri::shader_collection& Shaders)
	{
		...
		const double frame_delta = 1.0 / boost::any_cast<double>(k3d::property::pipeline_value(document().pipeline(), *frame_rate_property));
		const double frame_time = boost::any_cast<double>(k3d::property::pipeline_value(document().pipeline(), *time_property));
		...
		// Crop window ...
		const double crop_left = boost::any_cast<double>(k3d::property::pipeline_value(document().pipeline(), Camera.crop_window().crop_left()));
		const double crop_right = boost::any_cast<double>(k3d::property::pipeline_value(document().pipeline(), Camera.crop_window().crop_right()));
		const double crop_top = boost::any_cast<double>(k3d::property::pipeline_value(document().pipeline(), Camera.crop_window().crop_top()));
		const double crop_bottom = boost::any_cast<double>(k3d::property::pipeline_value(document().pipeline(), Camera.crop_window().crop_bottom()));
