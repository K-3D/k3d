...
void select_connected_components(k3d::polyhedron::primitive& Polyhedron, k3d::mesh::selection_t& PointSelection)
{
	...
	k3d::polyhedron::create_edge_adjacency_lookup(Polyhedron.edge_points, Polyhedron.clockwise_edges, boundary_edges, adjacent_edges);
	...
	for(k3d::uint_t face = 0; face != face_count; ++face)
	{
		...
		for(k3d::uint_t loop = loop_begin; loop != loop_end; ++loop)
		{
			...
			for(k3d::uint_t edge = first_edge; ;)
			{
				...
				if(PointSelection[Polyhedron.edge_points[edge]])
	...
	for(k3d::uint_t i = 0; i != point_selected_edges.size(); ++i)
		select_connected_points(tagged_edges, PointSelection, Polyhedron.edge_points, Polyhedron.clockwise_edges, adjacent_edges, point_selected_edges[i]);
}
