...
public:
	select_face_by_number(k3d::iplugin_factory& Factory, k3d::idocument& Document) :
		...
		m_index(init_owner(*this) + init_name("index") + init_label(_("Face Index")) + init_description(_("Face Index")) + init_value(0L) + init_step_increment(1) + init_units(typeid(k3d::measurement::scalar)) + init_constraint(constraint::minimum(0)))
	{
