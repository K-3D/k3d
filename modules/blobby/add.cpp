...
public:
	...
	void on_create_mesh_topology(k3d::mesh& Mesh)
	{
			...
			if(property.property_type() == typeid(k3d::mesh*))
			{
				...
				for(k3d::uint_t blobby = blobby_begin; blobby != blobby_end; ++blobby)
				{
					...
					if(source_primitive_count || source_operator_count)
						roots.push_back(target_primitive_counts[0] + target_operator_counts[0] - 1);
				}
			}
		}	
		...
		if(!roots.empty())
		{
			...
			target_operator_first_operands.push_back(target_operands.size());
			target_operator_operand_counts.push_back(roots.size() + 1);
			target_operands.push_back(roots.size());
			target_operands.insert(target_operands.end(), roots.begin(), roots.end());
		}
	}
