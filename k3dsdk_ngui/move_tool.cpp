...
const k3d::icommand_node::result move_tool::execute_command(const std::string& Command, const std::string& Arguments)
{
...
iuser_input_model& move_tool::get_input_model()
{
	return input_model();
}

void move_tool::record_command(viewport::control& Viewport, const GdkEventButton& Event, const bool Move)
{
...
void move_tool::record_transform(viewport::control& Viewport, const GdkEventMotion& Event, const k3d::vector3& Move)
{
	command_arguments arguments;
	arguments.append_viewport_coordinates("mouse", Viewport, Event);
