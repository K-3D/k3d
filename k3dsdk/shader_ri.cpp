...
parameter_list shader::shader_arguments(const render_state& State)
{
	...
	for(iproperty_collection::properties_t::const_iterator prop = properties.begin(); prop != properties.end(); ++prop)
	{
		if(dynamic_cast<iuser_property*>(*prop))
		{
			...
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::real>(property::pipeline_value(document().dag(), property))));
			}
			else if(property_type == typeid(ri::string))
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::string>(property::pipeline_value(document().dag(), property))));
			}
			...
			{
				if(k3d::ri::itexture* const texture = dynamic_cast<k3d::ri::itexture*>(boost::any_cast<k3d::inode*>(property::pipeline_value(document().dag(), property))))
			...
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::point>(property::pipeline_value(document().dag(), property))));
			}
			else if(property_type == typeid(ri::vector))
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::vector>(property::pipeline_value(document().dag(), property))));
			}
			else if(property_type == typeid(ri::normal))
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::normal>(property::pipeline_value(document().dag(), property))));
			}
			else if(property_type == typeid(ri::hpoint))
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::hpoint>(property::pipeline_value(document().dag(), property))));
			}
			else if(property_type == typeid(ri::matrix))
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::matrix>(property::pipeline_value(document().dag(), property))));
			}
			else if(property_type == typeid(ri::color))
			{
				results.push_back(parameter(property.property_name(), CONSTANT, 1, boost::any_cast<ri::color>(property::pipeline_value(document().dag(), property))));
			}
			else
			{
