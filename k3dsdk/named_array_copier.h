...
public:
	...
	/// Initializes named_array_copier to copy data from a collection of source arrays to a collection of target arrays, using a copy_policy
	/// object to determine how each source array maps to each target array.
	named_array_copier(const named_arrays& Source, named_arrays& Target, const copy_policy& CopyPolicy = strict_copy());
	~named_array_copier();
