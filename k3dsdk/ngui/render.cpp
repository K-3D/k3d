...
void render_animation(document_state& DocumentState, k3d::ianimation_render_engine& Engine)
{
	...
	const double start_time = boost::any_cast<double>(k3d::property::pipeline_value(DocumentState.document().pipeline(), *start_time_property));
	const double end_time = boost::any_cast<double>(k3d::property::pipeline_value(DocumentState.document().pipeline(), *end_time_property));
	const double frame_rate = boost::any_cast<double>(k3d::property::pipeline_value(DocumentState.document().pipeline(), *frame_rate_property));
